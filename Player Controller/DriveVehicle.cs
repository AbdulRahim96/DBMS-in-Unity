...
public class DriveVehicle : MonoBehaviour
{
        ...
        if (other.CompareTag("HandleTrigger"))
        {
                ...
                || (GameControl.manager.controlMode == ControlMode.touch && GameControl.driving == true))
            {
                ...
                if (other.transform.root.GetComponent<CarComponents>())
                {
                    ...
                    if(!carComponents.hasStolen)
                    {
                        if (Player.CNIC != 0) // if the user is registered in database while stealing
                        {
                            ...
                            SQLConnect.ShowMessage("Car has been stolen by " + Player.username, Color.red, 2);
                            int fine = 1000;
                            int cnic = Player.CNIC;
                            WWWForm form = new WWWForm();
                            form.AddField("table", "policeStation");
                            form.AddField("column1", cnic);
                            form.AddField("column2", "Stole Car");
                ...
                if (other.transform.root.GetComponent<BikeComponents>())
                {
                    ...
                    if (!bikeComponents.hasStolen)
                    {
                        if (Player.CNIC != 0)
                        {
                            // stealing bike panalty
                            bikeComponents.hasStolen = true;
                            SQLConnect.ShowMessage("Bike has been stolen by " + Player.username, Color.red, 2);
                            int fine = 800;
                            int cnic = Player.CNIC;
                            WWWForm form = new WWWForm();
                            form.AddField("table", "policeStation");
                            form.AddField("column1", cnic);
                            form.AddField("column2", "Stole Bike");
                            form.AddField("column3", fine);
                            form.AddField("column4", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
                            SQLConnect.InsertRecord(form);
                        }
    ...
    void OnTriggerExit(Collider other)
    {
        if (other.CompareTag("HandleTrigger"))
        {
            ...
            {
                if (Player.CNIC != 0)
                   Player.StartCoroutine(Player.crimeCheck());
                else
